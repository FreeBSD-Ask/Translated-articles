...
在编译时会显示对话框，你可以以交互方式选择这些定制项的启用或禁用。具体来说，可以通过以下命令进行配置，这些配置项如下：【注 3、4】。
...
另外，如果你想要恢复默认设置，可以使用 `make rmconfig` 命令，这将会清除在对话框中进行的配置。

## 全局设置

你可以通过在 `/etc/make.conf` 文件中进行配置，从而全局性地定义定制内容（开启或关闭）。
...
在 `/etc/make.conf` 文件中，你还可以进一步针对各个 Port 进行定制设置。

例如，基本上你可能想要安装文档类文件，但在为了安装这些文档而开始安装 TeX 的情况下，你可能会感到不满意，希望不安装文档。在这种情况下，你可以通过 `${OPTIONS_NAME}_SET`或`${OPTIONS_NAME}_UNSET` 来定义。
...
例如，在以下示例中，如果在 `/usr/ports/databases/sqlite3` 目录中运行 `/usr/local/libexec/portconf`，你将会看到显示出 `|NO_DIALOG=yes` 的内容。
...
当然，你也可以在 ports.conf 中编写 `OPTIONS_SET`／`OPTIONS_UNSET`，而不使用 `${OPTIONS_NAME}_SET`／`${OPTIONS_NAME}_UNSET`。但是需要注意，如果在未安装 portconf 的环境中进行构建，其影响可能会很大。因此，更好的做法是在影响不大的地方熟练地使用它。

如果你理解了上述内容，对于其他指定方式，你可以执行 `pkg info -D portconf`，然后阅读内容来进行编写。
...
如果你想要快速反映变更，建议使用 Port 编译，但请注意不要进行定制。
...
答：没关系，没问题。你的理解是正确的。虽然确实需要做出一些解释，但我有点累了，所以我要睡觉了。
