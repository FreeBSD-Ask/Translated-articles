...
在当前的 FreeBSD 环境中，不仅基本系统，连第三方的二进制包的使用（安装）也在持续增加。事实上，如果只是进行轻量级的设置和使用，已经有足够多的二进制包可供选择了。

然而，就我个人而言，由于我进行了大量的定制以适应我的需求，我无法满足于仅仅使用二进制包……这就是为什么我整理了有关自定义 Ports Collection 的方法。

至于那些想知道'什么是 ports（FreeBSD ports）'的读者，请参考所引用的资料。再次抱歉。"

## 适用环境

由于已经过去了五年以上，到了 2020 年，因此在目前支持的所有 FreeBSD 环境中都可以使用。大致可以考虑从版本 10 开始的版本。"
...
特别是，一旦在对话框中确认了选项，只有在明确执行 make config 命令或者在新增选项时才会再次显示对话框。
...
请注意，"カスタマイズ無し" 、"ダイアログの表示無し"和其他上下文中的翻译会因语境不同而有所变化。
