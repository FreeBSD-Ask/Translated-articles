...
不知道你怎么样，但我经常更新我的包……而且数量很多，实际上一千多个。
...
与其创建符号链接——这种方法会全局生效——你也可以为 **/usr/local/share/chromium/chrome** 二进制文件单独创建一个合适的 **[libmap.conf](https://man.freebsd.org/libmap.conf)** 配置文件。
...
……一旦你安装了 **bsdadminscripts2**，就无法再安装 **bsdadminscripts**，因为它们存在冲突。我之前已经安装了 **bsdadminscripts2**，但想在系统中添加 **bsdadminscripts**。
...
下面是 **/usr/ports/ports-mgmt/bsdadminscripts2** Port/包的描述。
